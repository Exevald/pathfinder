...
unsigned int Mesh::GetVertexBufferOffset() const
{
...
unsigned Mesh::AddSubMesh(unsigned startIndex, unsigned subMeshIndexCount)
{
	...
	if (
		(subMeshIndexCount == 0) || (startIndex + subMeshIndexCount < startIndex) || (startIndex + subMeshIndexCount > m_indexCount))
	{
...
{
	return (m_indexType == GL_UNSIGNED_BYTE) ? sizeof(GLubyte) : (m_indexType == GL_UNSIGNED_SHORT) ? sizeof(GLshort)
